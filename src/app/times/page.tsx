...
export default function Times() {
  ...
  useEffect(() => {
    leagueList?.map((leagues: any) => {
      if(leagues.league.id.toString() === league) {
  ...
  if(countriesError || error) {
    return (
      <div className="w-full h-screen flex items-center justify-center overflow-auto pb-20 bg-[#223142] text-white px-7">
        <span>Não foi possivel encontrar os dados, entre em contato com o suporte</span>
      </div>
    )
  }
  ...
  return (
    <div className="w-full h-screen overflow-auto pb-20 bg-[#223142] text-white px-7">
      ...
      <div className='md:flex md:justify-between'>
        ...
        <TeamsShields country={pais} leagueId={league} season={season} />
      </div>
    </div>
  )
}